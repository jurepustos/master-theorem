...
lemma ceilDiv_eq_div_or_div_succ {a b : ℕ} (hb : b > 0) :
...
theorem ceilDiv_ceilDiv_of_mul_dvd {a b c : ℕ} (hbc : b * c > 0)
    (hdvd : (b * c) ∣ a) : a ⌈/⌉ b ⌈/⌉ c = a / (b * c) := by
...
theorem ceilDiv_ceilDiv_le_of_right_dvd {a b c : ℕ} (hb : b > 0) (hc : c > 1)
    (hdvd : c ∣ a ⌈/⌉ b) : a ⌈/⌉ b ⌈/⌉ c ≤ a ⌈/⌉ (b * c) + 1 := by
...
lemma le_ceilDiv_ceilDiv {a b c : ℕ} (hb : b > 0) (hc : c > 1) :
    a ⌈/⌉ (b * c) - 1 ≤ a ⌈/⌉ b ⌈/⌉ c := by
  have c_pos : c > 0 := lt_trans one_pos hc
  have bc_pos := mul_pos hb c_pos
...
lemma ceilDiv_ceilDiv_le {a b c : ℕ} (hb : 0 < b) (hc : 0 < c) :
    a ⌈/⌉ b ⌈/⌉ c ≤ a ⌈/⌉ (b * c) := by
  rw [ceilDiv_le_iff_le_mul hc, ceilDiv_le_iff_le_mul hb, ← mul_assoc, 
      mul_comm, mul_comm, ← smul_eq_mul]
  apply le_smul_ceilDiv
  exact mul_pos hb hc
