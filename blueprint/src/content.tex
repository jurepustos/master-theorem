...
such that $|f(x)| \le k*|g(x)|$ for all $x \gt x_0$. Mind the universal quantification over $k$ as
opposed to existential quantification in prior definitions. While $g(x) \in O(f(x))$ and implies
$\limsup_{x \to \infty} \frac{|f(x)|}{|g(x)|} \lt \infty$, $g(x) \in o(f(x))$ implies 
...
if for all positive $k$ there exists $x_0$ such that $|f(x)| \ge k*|g(x)|$ for all $x \gt x_0$.
And just like before, while $g(x) \in \Omega(f(x))$ implies $liminf_{x \to \infty} \frac{|f(x)|}{|g(x)|} \gt 0$,
$g(x) \in \omega(f(x))$ implies $lim_{x \to \infty} \frac{|f(x)|}{|g(x)|} = \infty$.

\subsubsection Goals (to be updated as necessary):
\begin{itemize}
